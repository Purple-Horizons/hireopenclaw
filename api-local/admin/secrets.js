...
const DEV_DEFAULT_KEY = 'clawops-local-dev-key-change-in-production';
if ((!ENV_KEY || ENV_KEY === DEV_DEFAULT_KEY) && process.env.NODE_ENV === 'production') {
  console.error('FATAL: SECRETS_ENCRYPTION_KEY must be set to a non-default value in production');
  process.exit(1);
}
if (!ENV_KEY) {
